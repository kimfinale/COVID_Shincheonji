...
### FIT ABC SMC two-group SEIR tauleap
```{r}
rm(list=ls())
library(tmvtnorm)
library(truncnorm)
library(Rcpp) 
sourceCpp( "func/seir_2grp_tauleap_cpp.cpp" )
...
write.csv( params, paste0("outputs/params_", tstamp, ".csv"), row.names=FALSE )
...
library(truncnorm)
library(Rcpp) 
sourceCpp( "codes/seir_2grp_tauleap_cpp.cpp" )
source( "codes/read_inputs.R" )
...
```{r}
# tstamp <- "0522T1228"
tstamp <- "0614T2133" # shorter latent period
# tstamp <- "0614T1722"
...
quantile( round(p$V2), probs=pr )
quantile( p$V3, probs=pr )
quantile( p$V4, probs=pr )
...
d <- cbind( 
  data.frame( 
    par = c("R01", "I0", "Mixing", "Final_R0", "Intervention days", "R02"),
    id = (6-c(1, 4, 6, 3, 5, 2))),
  as.data.frame( t(pq) ) )
...
plt <- ggplot(d) +
  ...
  scale_x_continuous( name="Parameters",labels=c(expression(italic(c)~"*"), expression(italic(d)), expression(italic(I)[0]), expression(italic(R)~"*"), expression(italic(R)[0]^2), expression(italic(R)[0]^1)) ) + 
...
# xx <- readRDS( paste0( "outputs/seir_2grp_tauleap_", tstamp, ".rds" ) )
...
for( ii in seq_len(nrow(par)) ){
  ...
  I01 <- (1-params$frac_asymp) * as.double( par[ ii, 2 ] )
  A01 <- params$frac_asymp * as.double( par[ ii, 2 ] )
  params$init$I1 <- round( I01 )
  params$init$A1 <- round( A01 )
  params$init$S1 <- params$pop_high - I01
  mix <- as.double( par[ ii, 3 ] )
...
ggplot( low ) +
  geom_ribbon( aes(x=date, ymin=q025, ymax=q975), fill="blue",  alpha=0.2) +
  geom_ribbon( aes(x=date, ymin=q250, ymax=q750), fill="blue",  alpha=0.4) +
  geom_line( aes(date, q500), color="blue", size=1.2 ) +
  # geom_ribbon( data=high, aes(x=date, ymin=q025, ymax=q975), fill="red", alpha=0.2, inherit.aes=FALSE) +
  # geom_ribbon( data=high, aes(x=date, ymin=q250, ymax=q750),  fill="red", alpha=0.4, inherit.aes=FALSE) +
  # geom_line( data=high, aes(date, q500), color="red", size=1.2, inherit.aes=FALSE ) +
  geom_point( data=DAT, aes(date, cumul_low), color="blue", inherit.aes=FALSE ) +
  # geom_point( data=DAT, aes(date, cumul_shincheonji), color="red", inherit.aes=FALSE ) +
  labs(title="Non-Shincheonji", x="Date", y="Cumulative number of cases") +
  scale_x_date()

# tstamp <- format( Sys.time(), "%m%dT%H%M" )
# ggsave( paste0("plots/low_risk_cumul_", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )  
  
ggplot( high ) +
  geom_ribbon( data=high, aes(x=date, ymin=q025, ymax=q975), fill="red", alpha=0.2, inherit.aes=FALSE) +
  geom_ribbon( data=high, aes(x=date, ymin=q250, ymax=q750),  fill="red", alpha=0.4, inherit.aes=FALSE) +
  geom_line( data=high, aes(date, q500), color="red", size=1.2, inherit.aes=FALSE ) +
  geom_point( data=DAT, aes(date, cumul_shincheonji), color="darkred", inherit.aes=FALSE ) +
  labs(title="Shincheonji", x="Date", y="Cumulative number of cases") +
  scale_x_date()

# ggsave( paste0("plots/high_risk_cumul_", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )  
...
ggplot( d_low ) +
  ...
  geom_ribbon( aes(x=date, ymin=q250, ymax=q750), fill="blue",  alpha=0.4) +
  geom_line( aes(date, q500), color="blue", size=1.2 ) +
  # geom_ribbon( data=d_high, aes(x=date, ymin=q025, ymax=q975), fill="red", alpha=0.2, inherit.aes=FALSE) +
  # geom_ribbon( data=d_high, aes(x=date, ymin=q250, ymax=q750),  fill="red", alpha=0.4, inherit.aes=FALSE) +
  # geom_line( data=d_high, aes(date, q500), color="red", size=1.2, inherit.aes=FALSE ) +
  geom_point( data=DAT, aes(date, daily_low), inherit.aes=FALSE ) +
  geom_line( data=DAT, aes(date, daily_low), inherit.aes=FALSE ) +
  # geom_point( data=DAT, aes(date, daily_shincheonji), color="red", inherit.aes=FALSE ) +
  labs(title="Non-Shincheonji",x="Date", y="Daily confirmed case") +
  scale_x_date() +
  scale_y_continuous(limits=c(0,550)) +
  annotate( "segment", x=ymd("2020-03-23"), xend=ymd("2020-03-25"), y=500, yend=500, size=1 ) +
  annotate( "point", x=ymd("2020-03-24"), y=500, size=1.4 ) +
  annotate( "text", x=ymd("2020-03-26"), y=500, size=4, label="Data", hjust=0, vjust=0.5 )+
  annotate( "segment", x=ymd("2020-03-23"), xend=ymd("2020-03-25"), y=475, yend=475, size=1, color="blue" ) +
  annotate("rect", xmin=ymd("2020-03-23"), xmax=ymd("2020-03-25"), ymin=465, ymax=485, alpha=0.2, fill="blue") + 
  annotate("rect", xmin=ymd("2020-03-23"), xmax=ymd("2020-03-25"), ymin=470, ymax=480, alpha=0.4, fill="blue") + 
  annotate( "text", x=ymd("2020-03-26"), y=475, size=4, label="Model", hjust=0, vjust=0.5 ) +
  annotate( "text", x=ymd("2020-02-07"), y=550, label="B", hjust=0, vjust=0.5, size=5 )

  
# tstamp <- format( Sys.time(), "%m%dT%H%M" )
# ggsave( paste0("plots/low_risk_daily_", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )

ggplot( d_high ) +
  geom_ribbon( aes(x=date, ymin=q025, ymax=q975), fill="red",  alpha=0.2) +
  geom_ribbon( aes(x=date, ymin=q250, ymax=q750), fill="red",  alpha=0.4) +
  ...
  annotate( "point", x=ymd("2020-03-24"), y=500, size=1.4 ) +
  annotate( "text", x=ymd("2020-03-26"), y=500, size=4, label="Data", hjust=0, vjust=0.5 )+
  annotate( "segment", x=ymd("2020-03-23"), xend=ymd("2020-03-25"), y=475, yend=475, size=1, color="red" ) +
  annotate("rect", xmin=ymd("2020-03-23"), xmax=ymd("2020-03-25"), ymin=465, ymax=485, alpha=0.2, fill="red") + 
  annotate("rect", xmin=ymd("2020-03-23"), xmax=ymd("2020-03-25"), ymin=470, ymax=480, alpha=0.4, fill="red") + 
  annotate( "text", x=ymd("2020-03-26"), y=475, size=4, label="Model", hjust=0, vjust=0.5 ) +
  annotate( "text", x=ymd("2020-02-07"), y=550, size=5, label="A", hjust=0, vjust=0.5 )

# ggsave( paste0("plots/high_risk_daily_", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )

#library(gridExtra)
#p <- grid.arrange( p1, p2, p3, p4, nrow = 4)
#p
# text( ymd("2020-03-1"), 1.2e4, paste0("Outbreak size (median) = ", formatC(q500, format="d", big.mark=',') ), pos=4 )
# text( ymd("2020-03-1"), 1.1e4, paste0("Outbreak size (50%) = ", formatC(q250, format="d", big.mark=','), ", ",  formatC(q500, format="d", big.mark=',')), pos=4  )
# text( ymd("2020-03-1"), 1.0e4, paste0("Outbreak size (95%) = ", formatC(q025, format="d", big.mark=','), ", ",  formatC(q975, format="d", big.mark=',')), pos=4  )

# total <- dplyr::filter( sim, pop == 3 )
# high <- dplyr::filter( sim, pop == 1 )


### Figure 3A. Comparision of cumulative number of cases 
...
# params <- params_est[["params"]]
# 
rrxa <- params$rel_rate_isol_asymp
base_rate <- params$rate_isol
base_rate_asymp <- params$rate_isol*rrxa
max_rate <-params$max_rate_isol
gamma <- params$gamma
fa <- params$frac_asymp
base_dur <- (1-fa)/(gamma+base_rate) + fa/(gamma+base_rate*rrxa);
np <- nrow(par)
rate <- matrix( rep( base_rate, nr*np ), ncol=np )
rate_asymp <- matrix( rep( base_rate_asymp, nr*np ), ncol=np )
R1 <- matrix( rep( NA, nr*np ), ncol=np )
R2 <- matrix( rep( NA, nr*np ), ncol=np )
beta1 <- matrix( rep( NA, nr*np ), ncol=np )
beta2 <- matrix( rep( NA, nr*np ), ncol=np )

dur <- matrix( rep( base_dur, nr*np ), ncol=np )
t_int_start <- params$time_intervention_start
 # c("R01", "I0", "Mixing", "Final_R01", "Intervention days", "R02"), 
...
for( ii in 1:nrow(par) ){
  dur_int <- as.double( par[ ii, 5 ] )
  t_int_stop <- dur_int + t_int_start
  b1 <- as.double( par[ ii, 1 ] ) / base_dur
  b2 <- as.double( par[ ii, 6 ] ) / base_dur 
  mix <- as.double( par[ ii, 3 ] )
  cm <- c( 1 - mix, mix, mix, 1-mix )
  final_b1 <- as.double( par[ ii, 4 ] ) / base_dur
  final_b2 <- as.double( par[ ii, 4 ] ) / base_dur
  for( tt in 1:nr ){  
    time_diff <- tt - t_int_start;
    rrxa <- params$rel_rate_isol_asymp
    beta1[tt,ii] = b1
    beta2[tt,ii] = b2 
    R1[tt,ii] = as.double( par[ ii, 1 ])
    R2[tt,ii] = as.double( par[ ii, 6 ])
    if( 0 < time_diff & time_diff <= dur_int ){
      rate[tt, ii] <- base_rate + time_diff*(max_rate - base_rate)/dur_int;
      rrxa <- time_diff/dur_int;
      rate_asymp[tt, ii] <- 1/(rate[tt, ii]*rrxa)
      beta1[tt,ii] = b1 - time_diff*(b1 - final_b1)/dur_int;
      beta2[tt,ii] = b2 - time_diff*(b2 - final_b2)/dur_int;
      # patch1 = beta1_t*( I1*cm[0]+I2*cm[2] )/( N1*cm[0]+N2*cm[2] );
      # patch2 = beta2_t*( I1*cm[1]+I2*cm[3] )/( N1*cm[1]+N2*cm[3] );
      # new_inf1 = iS1*( cm[0]*patch1 + cm[1]*patch2 );
      # new_inf2 = iS2*( cm[2]*patch1 + cm[3]*patch2 );
    }
    else if( dur_int < time_diff ){
      rrxa <- 1
      rate[tt, ii] <- max_rate
      rate_asymp[tt, ii] <- max_rate
      beta1[tt,ii] = final_b1
      beta2[tt,ii] = final_b2
    }
    dur[tt, ii] <- (1-fa)/(gamma+rate[tt, ii]) + fa/(gamma+rate[tt, ii]*rrxa)
    R1[tt,ii] = beta1[tt,ii] * dur[tt, ii]
  }
}
...
new_inf_high <- CI_high 
new_inf_low <- CI_low  
new_inf_high[ 2:nr, ] <- CI_high[ 2:nr, ] - CI_high[ 1:(nr-1), ]
new_inf_low[ 2:nr, ] <- CI_low[ 2:nr, ] - CI_low[ 1:(nr-1), ] 

Rt_high <- matrix( nrow=nr, ncol=nc )
Rt_low <- Rt_high
Rt_tot <- Rt_high
new_inf_tot <- new_inf_low + new_inf_high

I_tot <- I_low + I_high

# beta1*I_high*(N-CI)/N = new_inf_high
beta1 = new_inf_high/I_high*N/(N-CI)
Rt1 = beta1*dur

for( ii in 1:nr ){
  for( jj in 1:nc ){
    # if(  I_high[ ii, jj ] > 0 ) {
    #   Rt_high[ii,jj] <- dur[ii,jj] * new_inf_high[ii,jj] / I_high[ii,jj] * params$pop_high/(params$pop_high-CI_high[ii,jj])
    # }
    # if( I_low[ ii, jj ] > 0 ){
    #   Rt_low[ii,jj] <- dur[ii,jj] * new_inf_low[ii,jj] / I_low[ ii, jj ] * params$pop_low/(params$pop_low-CI_low[ii,jj])
    # }
    # if( I_tot[ ii, jj ] > 0 ){
    #   Rt_tot[ii,jj] <- dur[ii,jj] * new_inf_tot[ii,jj] / I_tot[ ii, jj ] * (params$pop_high+params$pop_low)/(params$pop_high+params$pop_low-CI_high[ii,jj]-CI_low[ii,jj])
    # }

  Rt_high[ii,jj] <- dur[ii,jj]*beta1[ii,jj]
  Rt_low[ii,jj] <- dur[ii,jj]*beta2[ii,jj]
  Rt_tot[ii,jj] <- dur[ii,jj]*beta1[ii,jj]
  }
}
...
#      else{
#      }
#      Rt_tot[ ii, jj ] <- (1/params$rate_isol)*sum(new_inf_tot[ (ii-x):ii, jj ]) / sum(I_tot[ (ii-x):ii, jj ])
#     }
#   }
# }

Rt_high_summary <- apply( Rt_high, 1, quantile, probs=c(0.025, 0.25, 0.5, 0.75, 0.975), na.rm=TRUE )
Rt_low_summary <- apply( Rt_low, 1, quantile, probs=c(0.025, 0.25, 0.5, 0.75, 0.975), na.rm=TRUE )
Rt_tot_summary <- apply( Rt_tot, 1, quantile, probs=c(0.025, 0.25, 0.5, 0.75, 0.975), na.rm=TRUE )

Rt_summary <- data.frame( date=dates, 
                        q500_hr=Rt_high_summary[3, ], 
                        q025_hr=Rt_high_summary[1, ],
                        q975_hr=Rt_high_summary[5, ], 
                        q250_hr=Rt_high_summary[2, ], 
                        q750_hr=Rt_high_summary[4, ],
                        
                        q500_lr=Rt_low_summary[3, ], 
                        q025_lr=Rt_low_summary[1, ],
                        q975_lr=Rt_low_summary[5, ],
                        q250_lr=Rt_low_summary[2, ],
                        q750_lr=Rt_low_summary[4, ],
                        
                        q500_tot=Rt_tot_summary[3, ], 
                        q025_tot=Rt_tot_summary[1, ],
                        q975_tot=Rt_tot_summary[5, ],
                        q250_tot=Rt_tot_summary[2, ],
                        q750_tot=Rt_tot_summary[4, ])

library(ggplot2)
ggplot( Rt_summary[-1,] ) +
  geom_ribbon( aes(x=date, ymin=q025_hr, ymax=q975_hr), fill="red", alpha=0.2) +
  geom_ribbon( aes(x=date, ymin=q250_hr, ymax=q750_hr), fill="red", alpha=0.4) +
  geom_line( aes(x=date, y=q500_hr), color="red" ) +
  # geom_ribbon( aes(x=date, ymin=q025_lr, ymax=q975_lr), fill="blue", alpha=0.1) +
  # geom_line( aes(x=date, y=q500_lr), color="blue" ) +
  # geom_ribbon( aes(x=date, ymin=q025_tot, ymax=q975_tot), alpha=0.4) +
  # geom_line( aes(x=date, y=q500_tot), size=1.2 ) +
  geom_hline( yintercept=1, color='darkred', linetype = "dashed") +
  labs(title="Shincheonji", x="Date", y="Effective reproduction number") +
  scale_x_date() +
  scale_y_continuous( breaks=c(1,5,10,15,20), labels=c(1,5,10,15,20) ) +
  theme_bw(base_size = 14) + 
  annotate( "text", x=ymd("2020-02-07"), y=22, size=5, label="C", hjust=0, vjust=0.5 )

#  tstamp <- format( Sys.time(), "%m%dT%H%M" )
 # ggsave( paste0("plots/eff_R0_shincheonji", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )

 ggplot( Rt_summary[-1,] ) +
  geom_ribbon( aes(x=date, ymin=q025_lr, ymax=q975_lr), fill="blue", alpha=0.2) +
  geom_ribbon( aes(x=date, ymin=q250_lr, ymax=q750_lr), fill="blue", alpha=0.4) +
  geom_line( aes(x=date, y=q500_lr), color="blue" ) +
  geom_hline( yintercept=1, color='darkred', linetype = "dashed") +
  labs(title="Non-Shincheonji", x="Date", y="Effective reproduction number") +
  scale_x_date() +
  scale_y_continuous( breaks=c(1,5,10,15,20), labels=c(1,5,10,15,20) ) +
  theme_bw(base_size = 14)  
  # annotate( "text", x=ymd("2020-02-07"), y=22, size=5, label="C", hjust=0, vjust=0.5 )
...
#  tstamp <- format( Sys.time(), "%m%dT%H%M" )
 # ggsave( paste0("plots/eff_R0_low_risk_", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )
 
 ggplot( Rt_summary[-1,] ) +
  geom_ribbon( aes(x=date, ymin=q025_tot, ymax=q975_tot), alpha=0.4) +
  geom_line( aes(x=date, y=q500_tot), size=1 ) +
  geom_hline( yintercept=1, color='darkred', linetype = "dashed") +
  labs(title="Total", x="Date", y="Effective reproduction number") +
  scale_x_date() +
  scale_y_continuous( breaks=c(1,5,10,15,20,25), labels=c(1,5,10,15,20,25) ) +
  theme_bw(base_size = 14) + 
  annotate( "text", x=ymd("2020-02-07"), y=25, size=5, label="C", hjust=0, vjust=0.5 )

#  tstamp <- format( Sys.time(), "%m%dT%H%M" )
 # ggsave( paste0("plots/eff_R0", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )
```



### RUN 2-Group SEIR Tauleap (manual exploration)
```{r}
rm(list=ls())
library(tidyverse)
library(Rcpp) 
sourceCpp("codes/seir_2grp_tauleap_cpp.cpp")
# sourceCpp("codes/seir_3grp_tauleap_cpp.cpp")
source( "codes/read_inputs.R" )
source( "codes/load_data.R" )
source( "codes/model_functions.R" )
...
res_high <- as.data.frame( matrix(nrow=(params[["ndays"]]), ncol=nsim) )
res_low <- as.data.frame( matrix(nrow=(params[["ndays"]]), ncol=nsim) )
I_high <- as.data.frame( matrix(nrow=(params[["ndays"]]), ncol=nsim) )
I_low <- as.data.frame( matrix(nrow=(params[["ndays"]]), ncol=nsim) )
CI_high <- as.data.frame( matrix(nrow=(params[["ndays"]]), ncol=nsim))
CI_low <- as.data.frame( matrix(nrow=(params[["ndays"]]), ncol=nsim) )

## dates on the first column 
res_high[,1] <- seq( ymd("2020/02/7"), by="day", length.out=nrow(res_high) ) 
res_low[,1] <- res_high[,1] 

# date_change_start = ymd("2020-04-23")
# params$pop3_contact_start = which(res_high$V1 == date_change_start )
# params$base_behavior_start = params$pop3_contact_start 
# params$beta1 <- 1.4
# I01 <- 11
# params$init$I1 <- (1-params$frac_asymp)* I01
# params$init$A1 <- params$frac_asymp* I01
# params$init$S1 <- params$pop_high - I01
# mix <- 0.00075
# params$cm <- c( 1 - mix, mix, mix, 1-mix )
# params$time_intervention_stop <- params$time_intervention_start + 16
# params$min_beta1 <- 0.1
# params$min_beta2 <- 0.1
# params$min_delay <- 1
...
for( ii in 1:(nsim-1) ){
  ...
  res_high[ , ii+1 ] <- res$X1[ params$day_filter ]
  res_low[ , ii+1 ] <- res$X2[ params$day_filter ]
  I_high[ , ii+1 ] <- res$I1[ params$day_filter ]
  CI_high[ , ii+1 ] <- res$CI1[ params$day_filter ]
  I_low[ , ii+1 ] <- res$I2[ params$day_filter ]
  CI_low[ , ii+1 ] <- res$CI2[ params$day_filter ]
...
nr <- nrow( res_high )
res_tot <- res_high
res_tot[,2:nc] <- res_high[,2:nc] + res_low[,2:nc] # excluding dates on the first column
pr <- c(0.025, 0.25, 0.5, 0.75, 0.975)
  
res_tot_summary = apply( res_tot[,2:nc], 1, quantile, probs=pr )
res_high_summary = apply( res_high[,2:nc], 1, quantile, probs=pr )
res_low_summary = apply( res_low[,2:nc], 1, quantile, probs=pr )



sim <- data.frame(rbind(
    data.frame(pop = 1, date=res_high$V1, q025=res_high_summary[1,], 
               q250=res_high_summary[2,], q500=res_high_summary[3,],  
               q750=res_high_summary[4,], q975=res_high_summary[5,]),
    data.frame(pop = 2, date=res_high$V1, q025=res_low_summary[1,], 
               q250=res_low_summary[2,], q500=res_low_summary[3,],  
               q750=res_low_summary[4,], q975=res_low_summary[5,]),
    data.frame(pop = 3, date=res_high$V1, q025=res_tot_summary[1,], 
               q250=res_tot_summary[2,], q500=res_tot_summary[3,],
               q750=res_tot_summary[4,], q975=res_tot_summary[5,])))

daily_low <- res_low[ 2:nr, ] - res_low[ 1:(nr-1), ] 
daily_high <- res_high[ 2:nr, ] - res_high[ 1:(nr-1), ] 

d_high_summary = apply( daily_high[, 2:nc ], 1, quantile, probs=pr )
d_low_summary = apply( daily_low[ , 2:nc ], 1, quantile, probs=pr )

sim_daily <- data.frame(rbind(
    data.frame(pop = 1, date=res_high$V1[2:nr], q025=d_high_summary[1,], 
               q250=d_high_summary[2,], q500=d_high_summary[3,],  
               q750=d_high_summary[4,], q975=d_high_summary[5,]),
    data.frame(pop = 2, date=res_high$V1[2:nr], q025=d_low_summary[1,], 
               q250=d_low_summary[2,], q500=d_low_summary[3,],  
               q750=d_low_summary[4,], q975=d_low_summary[5,])　) )
...
high <- dplyr::filter( sim, pop =="High risk" )
ggplot( low ) +
  # geom_ribbon( aes(x=date, ymin=q025, ymax=q975), fill="blue",  alpha=0.1) +
  # geom_ribbon( aes(x=date, ymin=q250, ymax=q750), fill="blue",  alpha=0.2) +
  geom_line( aes(date, q500), color="blue", size=1.2 ) +
  # geom_ribbon( data=high, aes(x=date, ymin=q025, ymax=q975), fill="red", alpha=0.1, inherit.aes=FALSE) +
  # geom_ribbon( data=high, aes(x=date, ymin=q250, ymax=q750),  fill="red", alpha=0.2, inherit.aes=FALSE) +
  geom_line( data=high, aes(date, q500), color="red", size=1.2, inherit.aes=FALSE ) +
  geom_point( data=DAT, aes(date, low), color="blue", inherit.aes=FALSE ) +
  geom_point( data=DAT, aes(date, shincheonji), color="red", inherit.aes=FALSE ) +
...
ggplot( d_low ) +
  geom_line( aes(date, q500), color="blue", size=1.2 ) +
  geom_line( data=d_high, aes(date, q500), color="red", size=1.2, inherit.aes=FALSE ) +
  geom_point( data=DAT, aes(date, daily_low), color="blue", inherit.aes=FALSE ) +
  geom_point( data=DAT, aes(date, daily_shincheonji), color="red", inherit.aes=FALSE ) +
  labs(x="Date", y="Cumulative number of cases") +
  scale_x_date() +
...
# tstamp <- format( Sys.time(), "%m%dT%H%M" )
# ggsave( paste0("plots/return_to_normal_", date_change_start, "_", tstamp, ".png"), width=3.4*2, height=2.7*2, units="in" )
